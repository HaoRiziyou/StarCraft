	...
	bool proceedOneSec() {
		...
		if (startTime >= buildTime)
		{
			...
			else
			{
				...
				else if (producedName == "SiegeTank")
				{
					vector<techLabOnFactory>& b = buildedList->getTechlabOnfactoryList();
					for (techLabOnFactory &ba : b) {
...
class Builder {
public:
	Builder(Timer& t, Resources& r, BuildedList& b, vector<map<string, string>>& tbl, string productionBuilding, string unit, size_t Limitedtime, bool jsonBool = false, char obNum = 100) :timer(&t), resources(&r), toBuildList(tbl), buildedList(&b), productionStruct(productionBuilding), targetUnit(unit), maxTime(Limitedtime), json(jsonBool), uNumber(obNum) {}
	void build(int i,string& s) {
		...
		else if((producedBy == "Barracks") || (producedBy == "Barracks/BarracksTechLab/BarracksReactor") )
		{
			 ...
			 if (name == "Marine")
			{
				...
				producedID = "Marine";
				producedID += (to_string(buildedList->buildMarine()));
		...
		else if(producedBy == "Factory" || producedBy == "Factory/FactoryTechLab/FactoryReactor" )
		{
		...
		}else if (producedBy == "BarracksTechLab"){
			if (name == "Marauder")
			{
				producedID = "Marauder";
				producedID += (to_string(buildedList -> buildMarauder()));
			}
			for (techLabOnBarracks& n : buildedList->getTechLabOnBarracksList())
			{
				if (n.getStatus() == IDLE)
				{
					IDLEBuilding = NULL;
			...
			if (name == "SiegeTank")
			{
				producedID = "SiegeTank";
				producedID += (to_string(buildedList -> buildSiegeTank()));
			}
			for (techLabOnFactory& n : buildedList->getTechlabOnfactoryList())
			{
				if (n.getStatus() == IDLE)
				{
					IDLEBuilding = NULL;
        ...
        /*else if(producedBy == "FactoryTechLab")
		{
			...
			for (factory& n : buildedList->getFactoryList())
			{
				if (n.getStatus() == IDLE)
				{
					...
					proceedManager pM(IDLEBuilding, producedID, name, build_time, buildedList, eProvided);
					proceedList.push_back(pM);
					break;
				}
			}
		...
		double vespene = stoi(item["vespene"]);
		...
		if (!proceedList.empty()) {
			for (proceedManager& pM : proceedList) {
				if (pM.proceedOneSec())
				{
					...
					//cout<<resources->getSupply()<<endl;
					string pName = pM.getProducedName();
					if (json)buildEndEvent(pName, pM.getProducerID(), pM.getProducedID());
					//buildEndEvent(pM.getProducedName(), pM.getProducerID(), pM.getProducedID());
					producedIDs.push_back(pM.getProducedID());
					float& nowTime = timer->getCurrentTime();
					if (nowTime <= maxTime)
					{
						if (pName == "SCV") workers++;
						else if (pName == productionStruct) buildingNumber++;
						else if (pName == targetUnit) { 
							unitNumber++; 
							if ((time == 700) && (unitNumber == uNumber)) {
					...
					{
						if ((pName == targetUnit) && (afterSim == 255)) afterSim = nowTime - maxTime;
					}
	...
	void deleteProceedList() {
		//delete &proceedList;
		vector<proceedManager>().swap(proceedList);
	}
	tuple<char, char, size_t, char> getdata() {
		if (uNumber == 100) {
			return make_tuple(workers, buildingNumber, unitNumber, afterSim);
		}
		else { 
			return make_tuple(workers, buildingNumber, time, afterSim); }
	}
	~Builder() {
		//delete[] c;
	}

...
class PreAction {
public:
	PreAction(string techT):refinery(0), needVespene(0), techTree(techT){}
	...
	//dependency precheck
	bool preCheck(vector<string> &toBuildList) {
		string store;
			...
			{	
				...
				if (store == toBuild)
				{
					...
					for (int j = 0; j <= i; ++j) {
						...
						if ((M["produced_by"] == toBuildList[j]) || (M["produced_by"] == "CommandCenter/OrbitalCommand/PlanetaryFortress")||(M["produced_by"] == "CommandCenter")||(M["produced_by"] == "SCV")) { producerCheck = true; break; }
						else if(M["produced_by"] == "Barracks/BarracksTechLab/BarracksReactor" ){
							for(size_t j=0;j<i;++j){
								if( (toBuildList[j] == "Barracks")){ producerCheck = true; break;}
							}
						}else if(M["produced_by"] == "Starport/StarportTechLab/StarportReactor" ){
							for(size_t j=0;j<i;++j){
								if( (toBuildList[j] == "Starport")){ producerCheck = true; break;}
							}
						}else if(M["produced_by"] == "Factory/FactoryTechLab/FactoryReactor"  ){
							for(size_t j=0;j<i;++j){
								if( (toBuildList[j] == "Factory")){ producerCheck = true; break;}
							}
						}
					}
					...
					if (store != "")
					{
						...
						}else if(store == "BarracksTechLab"){
							for(size_t j=0;j<i;++j){
								if( (toBuildList[j] == "Barracks")){ existCheck = true; break;}
							}
						}
						else{
							for (int j = 0; j < i; ++j) {
	...
	string getProductionStruct(string targetUnit) {
		ifstream outfile(techTree, ifstream::in);
		string store = "";
		getline(outfile, store, '\n');
		getline(outfile, store, '\n');
		while (!outfile.eof())
		{
			...
			{
				for (char i = 0; i < 20; i++)getline(outfile, store, ',');
				break;
			}
		}
		outfile.close();
